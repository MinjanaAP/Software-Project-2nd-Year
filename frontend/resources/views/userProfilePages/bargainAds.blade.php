@extends('layouts.userProfileLayouts')
@section('content')

<section class="my-profile mt-5 " id="my-profile">
    <div class="container-lg mt-3">
        <div class="row justify-content-center align-items-start">

            <!-- ----------left side------------>
            <div class="profile d-flex d-sm-none align-items-center ms-3">
                <div class="profile-image me-3">
                    <img src="data:image/jpeg;base64," alt="Profile Picture" class="avatar">                </div>
                <div class="profile-info">
                    <p class="user-name m-0  d-sm-none">Welcome!!!</p>
                </div>
            </div>
            
            <div class="col-sm-4 col-xl-3">
                
                @include('userProfilePages.sideNavBar')
                

            </div>

            <!-- ----------right side---------- -->
            <div class="col-sm-8 col-xl-8 col-12 d-sm-flex">
                <div class="card col-12">
                    <h5 class="card-header">Bargains Ads</h5>
                    <div class="card-body" >
                        <div class="accordion" >
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                                    Your Bargains
                                    </button>
                                </h2>
                                <div id="collapseOne" class="accordion-collapse collapse show" data-bs-parent="#accordionExample">
                                    <div class="accordion-body" id="adsContainer">
                                    </div>
                                </div>
                                </div>
                                <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                                    Bargains Prices for Your Ads
                                    </button>
                                </h2>
                                <div id="collapseTwo" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                                    <div class="accordion-body" id="bargainPriceContainer">
                                    </div>
                                </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>    
            
            <!-- ----------right side end---------- -->

        </div>
    </div>
</section>
<script>
    var baseUrl = "{{ env('APP_BASE_URL') }}";
    $(document).ready(function(){
        $('#bargainAds').addClass('activeTag');

        const token = sessionStorage.getItem('token');
        $.ajax({
            url: baseUrl + '/api/my/getBargainAds',
            type: 'GET',
            headers: {
                'Authorization':'Bearer'+token
            },
            success: function(response) {
        if (response.message === 'success') {
            let adsContainer = $('#adsContainer'); // Assuming there's a container for ads in your HTML
            adsContainer.empty(); // Clear any previous ads
            
            response.data.forEach(function(item) {
                let freeAd = item.free_ad; // Assuming 'free_ad' is the related model
                
                let adHtml = 
                    '<div class="viewAds p-2">' +
                        '<div class="viewAd img">' +
                            '<img class="rounded mx-auto d-block" width="200px" height="200px" src="' + freeAd.image_1 + '" alt="jj">' +
                        '</div>' +
                        '<div class="adDetails my-2 p-2">' +
                            '<span class="badge text-bg-success mt-1 mb-2">'+ freeAd.status +'</span>' +
                            '<div class="row mb-0">' +
                                '<div class="col-6 d-flex justify-content-between ">' +
                                    '<h4 class="itemPrice fw-bold" id="price"> LKR.' + freeAd.price + '</h4>' +
                                    '<div>' +
                                        '<span class="badge text-bg-warning me-3 ms-1 ">' + freeAd.negotiable +'</span>' +
                                    '</div>' +
                                '</div>' +
                                '<div class="col-6 d-flex justify-content-end">' +
                                    '<i class="bi bi-heart px-2"></i>' +
                                    '<i class="bi bi-share-fill px-2"></i>' +
                                '</div>' +
                            '</div>' +
                            '<h4 class="itemTitle fw-bold" id="title">' + freeAd.title + '</h4>' +
                            '<p class="itemLocation text-secondary"><i class="bi bi-geo-alt-fill me-2"></i>' + freeAd.town + '</p>' +
                            '<button type="button" class="btn w-100" onclick="viewAd('+ freeAd.id +')"> View Ad <i class="bi bi-arrow-right-short ms-2"></i></button>' +
                        '</div>' +
                        '<div class="freeAdDetails my-2 p-2">' + // Add Free Ad details
                            '<h5 class="bargainPrice fw-bold">Bargain Price: LKR. ' + item.bargain_price + '</h5>' +
                            '<p class="description">' + item.description + '</p>' +
                        '</div>' +
                    '</div>';
                
                adsContainer.append(adHtml);
            });
        } else {
            alert(response.message); // Handle the case where no ads are found
        }
    },
    error: function(xhr, status, error) {
        console.error(xhr.responseText); 
    }

        });

        // get bargain price for my Ads
        $.ajax({
            url:baseUrl + '/api/my/getBargainPrice',
            type:'GET',
            headers : {
                'Authorization':'Bearer' + token
            },
            success:function(response){
                if(response.message === 'success'){
                    let bargainPriceContainer = $('#bargainPriceContainer')
                    bargainPriceContainer.empty();
                    response.data.forEach(function(item){
                        let freeAd = item.free_ad;
                        //let user = item.user;

                        var adHTML = 
                                    '<div class="viewAds p-2">' +
                                        '<div class="viewAd img">' +
                                            '<img class="rounded mx-auto d-block" width="200px" height="200px" src="' + freeAd.image_1 + '" alt="Ad Image">' +
                                        '</div>' +
                                        '<div class="adDetails my-2 p-2">' +
                                            '<span class="badge text-bg-success mt-1 mb-2">'+ freeAd.status +'</span>' +
                                            '<div class="row mb-0">' +
                                                '<div class="col-6 d-flex justify-content-between ">' +
                                                    '<h4 class="itemPrice fw-bold" id="price"> LKR.' + freeAd.price + '</h4>' +
                                                    '<div>' +
                                                        '<span class="badge text-bg-warning me-3 ms-1 ">' + freeAd.negotiable + '</span>' +
                                                    '</div>' +
                                                '</div>' +
                                                '<div class="col-6 d-flex justify-content-end">' +
                                                    '<i class="bi bi-heart px-2"></i>' +
                                                    '<i class="bi bi-share-fill px-2"></i>' +
                                                '</div>' +
                                            '</div>' +
                                            '<h4 class="itemTitle fw-bold" id="title">' + freeAd.title + '</h4>' +
                                            '<p class="itemLocation text-secondary"><i class="bi bi-geo-alt-fill me-2"></i>' + freeAd.town + '</p>' +
                                            '<button type="button" class="btn w-100" onclick="viewAd(' + freeAd.id + ')"> View Ad <i class="bi bi-arrow-right-short ms-2"></i></button>' +
                                        '</div>' +
                                        '<div class="freeAdDetails my-2 p-2">' + // Add Free Ad details
                                            '<h5 class="bargainPrice fw-bold">Bargain Price: LKR. ' + item.bargain_price + '</h5>' +
                                            '<p class="description">' + item.description + '</p>' +
                                        '</div>' +
                                        '<div class="userDetails my-2 p-2">' + // Add User details
                                            '<h5 class="userName fw-bold">User Name: ' + item.user.first_name + '</h5>' +
                                            '<p class="userEmail">Email: ' + item.user.email + '</p>' +
                                            '<p class="userPhone">Phone: ' + item.user.telephone_no_1 + '</p>' +
                                        '</div>' +
                        '</div>';

            $('#bargainPriceContainer').append(adHTML);
                    })
                }
            },
            error: function(xhr, status, error) {
                console.error(xhr.responseText);
            }
        })


    });

    function viewAd(itemId) {
            // Redirect to the product page
            window.location.href = `/productPage1/${itemId}`;
            
        }
</script>
@endsection

