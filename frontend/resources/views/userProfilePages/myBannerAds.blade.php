...
<script>
    ...
    $(document).ready(function(){
        ...
        function getmyAds(){
            ...
            $.ajax({
                url:baseUrl + '/api/getSpecificPaidAd',
                type:'GET',
                headers:{
            ...
            response.data.forEach(paidAd => {
                
                var adHTML = '<div class="viewAds p-2">' +
                        '<div class="viewAd img">' +
                        '<img class="rounded mx-auto d-block" width="200px" height="200px" src="' + paidAd.image + '" alt="Ad Image">' +
                        '</div>' +
                        '<div class="adDetails my-2 p-2">' +
                            '<span class="badge text-bg-success mt-1 mb-2">Paid Ad</span>' +
                            '<div class="row mb-0">' +
                                '<div class="col-6 d-flex justify-content-between ">' +
                                    '<h4 class="itemPrice fw-bold" id="price">Ad Type: ' + paidAd.paid_ad_type + '</h4>' +
                                '</div>' +
                            '</div>' +
                            '<div class="d-flex">' +
                                '<h4 class="itemTitle fw-bold me-2" id="title">' + paidAd.name + '</h4>' +
                            '</div>' +
                            '<p class="itemLocation text-secondary"><i class="bi bi-geo-alt-fill me-2"></i>' + paidAd.address + '</p>' +
                            '<p class="itemContact text-secondary"><i class="bi bi-telephone-fill me-2"></i>' + paidAd.number + '</p>' +
                            '<p class="itemEmail text-secondary"><i class="bi bi-envelope-fill me-2"></i>' + paidAd.email + '</p>' +
                            '<a href="' + paidAd.url + '"  target="_blank">'+paidAd.url+'</a>' +
                        '</div>' +
                    '</div>';
